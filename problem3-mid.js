...
function listUsersWithMovies(users, movies) {
  ...
  for (const user of users) {
    // Filtramos las películas vistas por el usuario actual
    const moviesUser = movies.filter((movie) => movie.userId === user.id);
    let rateTotal = 0;
    for (const movie of moviesUser) {
      rateTotal += movie.rate;
    }

    // Calculamos la calificación promedio dividiendo la suma entre la cantidad de películas
    const rateProm = moviesUser.length > 0 ? rateTotal / moviesUser.length : 0;
    ...
    const resultUser = {
      ...
      moviesView: moviesUser.map((movie) => movie.title),
      rateProm: rateProm.toFixed(2), // Limitamos a 2 decimales
    };

    // Agregamos el objeto a nuestro arreglo 
